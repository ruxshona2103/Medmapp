...
# ===============================================================
# 🏥 BEMORNI HAMKORGA YUBORISH (OPERATOR PANEL)
# ===============================================================
class AssignPatientToPartnerView(APIView):
    """
    🏥 Operator tomonidan bemorni tanlangan hamkorga yuborish.
    POST /api/applications/assign_partner/
    """
    permission_classes = [permissions.IsAuthenticated]

    @swagger_auto_schema(
        operation_summary="🏥 Bemorni hamkorga yuborish",
        operation_description="""
        Operator tomonidan tanlangan bemor klinikaga yuboriladi.
        Stage avtomatik ravishda "HUJJATLAR" bosqichiga o‘tadi.
        Telegram xabarnoma hozircha ishlatilmaydi (faqat log yozadi).
        """,
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            required=["patient_id", "partner_id"],
            properties={
                "patient_id": openapi.Schema(type=openapi.TYPE_INTEGER, description="Bemor ID"),
                "partner_id": openapi.Schema(type=openapi.TYPE_INTEGER, description="Hamkor (Partner) ID"),
                "comment": openapi.Schema(type=openapi.TYPE_STRING, description="Izoh (ixtiyoriy)"),
            },
        ),
        responses={
            200: openapi.Response("Muvaffaqiyatli yuborildi"),
    ...
    def post(self, request):
        ...
        if not patient_id or not partner_id:
            return Response(
                {"detail": "patient_id va partner_id majburiy."},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # ✅ Bemor allaqachon boshqa hamkorga biriktirilganini tekshirish
        if patient.assigned_partner and patient.assigned_partner != partner:
            return Response(
                {"detail": f"Bemor allaqachon {patient.assigned_partner.name} hamkoriga biriktirilgan."},
                status=status.HTTP_400_BAD_REQUEST
            )

        # ✅ Stage: "HUJJATLAR" bosqichini olish (⚠️ code -> code_name)
        stage = Stage.objects.filter(code_name="stage_documents").first()
        if not stage:
            return Response(
                {"detail": "stage_documents topilmadi (Stage jadvalini tekshiring)."},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        patient.stage = stage
        patient.save(update_fields=["assigned_partner", "stage", "updated_at"])

        # ✅ Tarixga yozish
        PatientHistory.objects.create(
            patient=patient,
            author=request.user,
            comment=comment or f"{partner.name} klinikaga yuborildi."
        )

        # ✅ Hozircha faqat log chiqariladi (telegram yo‘q)
        print(f"[LOG] Bemor '{patient.full_name}' → '{partner.name}' ga yuborildi. Bosqich: HUJJATLAR")
        ...
        return Response({
            ...
            "stage": stage.title,
            "message": "Bemor muvaffaqiyatli yuborildi."
        }, status=status.HTTP_200_OK)
